 <?php
	...
	class content_manager {
		...
		public function json_service_curriculist() {
			$aCurricula = getCurricula ( array (
					'curriculum' 
			) );
			drupal_json_output ( $aCurricula );
			die ();
		}
		/**
		...
		public function getCurricula($aTypes, $sLang = 'de') {
			...
			foreach ( $aTypes as $sType ) {
				...
				$query->entityCondition ( 'entity_type', 'taxonomy_vocabulary', '=' )->propertyCondition ( 'machine_name', $sType . '_%', 'LIKE' )->propertyCondition ( 'weight', '0', '<' );
