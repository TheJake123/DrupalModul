...
class overviewPDF extends TCPDF {
	...
	function createPDF($iVID) {
		...
		$this->SetTitle ( 'Curriculum ' . 'Curriculum Wirtschaftsinformatik ' . $aCurricula [0]->version );
		...
		$this->Ln ();
		$this->SetFontSize ( 20 );
		$this->MultiCell ( 0, 0, $aCurricula [0]->faculty, 0, 'C' );
		$this->SetFontSize ( 12 );
		$this->Ln ();
		$this->MultiCell ( 0, 0, 'Version ' . $aCurricula [0]->version, 0, 'C' );
		$this->printCurriculum ( $oCurriculum );
		$this->createTOCPage ();
		$this->Output ( 'LVA-Übersicht Wirtschaftsinformatik ' . $oCurriculum->type . ' ' . $oCurriculum->version . '.pdf', 'I' );
	}
	/**
...
EOT;
	...
	static function getCourses($currId) {
		$sURL = 'http://drupal.dafalias.com/stukowin/crclm/' . $currId; // 'http://www.json-generator.com/api/json/get/cbojpRpYXS';
		$aCourses = json_decode ( file_get_contents ( $sURL ) );
		foreach ( $aCourses as $oCourse )
